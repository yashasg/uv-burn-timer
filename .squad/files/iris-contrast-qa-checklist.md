...
| L1 cover photosens disclaimer Label text (`Photosensitizing medications, conditions, recent skin treatments, and pregnancy can make this estimate overstate your burn window.`) | __:1 | __:1 | __:1 | __:1 | ☐ | **Added by WI-bundleT (Iris L02 — Loop-14).** Re-measurement required: Bundle T decomposed the Label so the *text* now renders in `.foregroundStyle(.primary)` (was `.orange` — measured ~2.3–2.5:1 in Light Mode Std, WCAG SC 1.4.3 fail). The `exclamationmark.triangle` icon retains `.foregroundStyle(.orange)` as a decorative warning glyph (no text payload — SC 1.4.3 N/A; SC 1.4.11 3:1 floor still applies to the icon vs `.regularMaterial`). Health-critical surface for Asha (P4 Accutane / photosensitizer cohort). Measure (a) the text on the cover material, (b) the orange icon on the cover material, both Std and Increased. Pinned by `test_T1_photosensitizerLineLabelOnL1CoverUsesPrimaryTextColor`. |
