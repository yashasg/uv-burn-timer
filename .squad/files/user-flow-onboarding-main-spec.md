...
3. **Photosensitization L3 reach-back lockup** — *(reconciled 2026-05-21 Loop-10 WI-cc: the standalone yellow `Photosensitization loop banner` (320×40, copy `Meds or photosensitive conditions? Learn more` / `ProductCopy.photosensitizationBannerLabel`) ratified by WI-57 is **retired** in shipped code per K-1 cleanup + commit `9da54cf`. Source-text guard `MainScreenCleanupContractTests.test_O1_photosensitizationBannerSymbolAbsentFromAppViews` hard-fails CI if the banner symbol is reintroduced.)* The L3 reach-back load is now carried by **two co-rendered surfaces**: (a) the toolbar ⓘ `EstimateInfoButton` (top-bar trailing, `accessibilityIdentifier("EstimateInfoButton")`) → `AboutView(highlightEstimateApplicability: true)` → auto-scrolls to the `notForMeAnchor` photosensitizer enumeration (S1/S2/S3 guards from WI-i); and (b) the `PersistentFooter` `Informational only. Not medical advice.` Label-link (bottom safe-area inset) → same `AboutView` highlight anchor (C1 SaMD anchor per Plunder). The `ProductCopy.photosensitizationBannerLabel` constant is retained as AUDIT-ONLY in `auditCopySurfaces` for audit fidelity; it is not rendered at runtime. Plunder C3 floor (L3 photosensitizer reach-back from result surface) remains satisfied via the toolbar ⓘ path.
...
6. **Inputs row — Fitzpatrick + Location + SPF** — compact 44pt controls for `🧑 Type III`, `📍 Approx. 37.77, -122.42 ›`, and `SPF 30`, wrapped in a quiet `INPUTS` section header (uppercase `.caption.weight(.semibold).foregroundStyle(.secondary)` with `.accessibilityAddTraits(.isHeader)` and `accessibilityIdentifier("MainInputsRowHeader")`) so the row reads as inputs that drive the hero gauge (Ninth-loop WI-q). Location refreshes the UV lookup; SPF stays directly adjustable on the main screen. The Fitzpatrick chip was added to the main surface during the Seventh-loop K-5/K-6 UX cleanup (commit `c23ed4e`, ratified in `.squad/decisions.md` line ~701) — earlier spec drafts said Fitzpatrick lived only in onboarding + Settings, but the K-5/K-6 decision reversed that and the chip is now the ambient tap target on repeat use. Tap on the chip opens `SkinTypeEditView` (when a type is committed) or returns to onboarding (when none is set). *(The location chip renders `UVCoordinate.privacyDisplayText` — a rounded two-decimal coordinate string — not a reverse-geocoded city name. No network call is made; no place name is stored or transmitted. Rationale: D-2026-05-19-002 zero-data architecture + D-2026-05-19-003 Apple Weather only.)*
...
2. 🚨 **L1 sticky + photo-sens reach-back** — arrow now points to the toolbar ⓘ `EstimateInfoButton` + the `PersistentFooter` link (Loop-10 WI-cc reconcile — the yellow `NowView` banner this callout originally pointed to was retired per K-1 / commit `9da54cf`; see LANE 2 #3). Both surfaces deep-link to `AboutView(highlightEstimateApplicability: true)`.
...
| 4 | **P4 Asha** (Accutane, Fitz IV) | purple #6A1B9A | ★ **LOAD-BEARING** visibility loop on L1; ★ **MOST IMPORTANT L3 TAP** on verdict (toolbar ⓘ `EstimateInfoButton` → `AboutView(highlightEstimateApplicability: true)`); ★ L1 re-fires on first install + `currentDisclaimerPolicyVersion` bump + foreground-with-elapsed-window (Loop-10 WI-cc reconcile — the Pattern-B persistence ratification 2026-05-21T07:00Z replaced the "every cold launch" model; see `.squad/designs/plunder-skin-type-persistence-floor.md` §C6 revised and `.squad/decisions/adr/` precedent). Mid-session Fitzpatrick chip tap does NOT re-fire L1 per the persistence floor's L1/Fitzpatrick decoupling. |
...
*Last reconciled with code: Loop-10 closure 2026-05-21 (WI-cc: spec drift + checklist coverage + decision archive — LANE 1 #2 Asha row reconciled to Pattern-B reality, LANE 2 #3 photosens banner retirement documented, LANE 3 callout 2 arrow updated, LANE 4 P4 Asha reattestation reconciled; cycle paired with WI-q `a30e25e` INPUTS header land, WI-o Group Z hero-summary estimate-language guards, WI-ee Group AA hero-gauge countdown-framing strip, WI-ff Group BB/GD Suchi persona P0 guards). Previous reconciliation: `da22498` — 2026-05-21 (WI-h: ADR-0001 hero card wrapper preserves toolbar hit-test — R7a/R7b causal-binding guards in BurnTimeCalculatorTests; cycle also shipped WI-g/WI-j dead-test wiring + CI membership guard `d1530a5`, WI-l ADR-0001 ratification `4efacbb`, WI-w L1 storage-disclosure sentence `626e261`, and the toolbar ⓘ EstimateInfoButton chrome-less hero card `8f2f16d`/`9da54cf`).*
