...
| 3 | Skin-Type Picker | light blue | blue | Fitzpatrick I–VI | Prompt: *"Choose by how your skin burns and tans, not by how it looks."* (rendered as `ProductCopy.skinTypePickerPrompt`). Subtext begins *"Pick the row that best matches what your skin does after about 30 minutes of midday summer sun, with no sunscreen and no recent tan. Each row covers a range of natural skin tones."* (`ProductCopy.skinTypePickerSubtext`). **NO default** (D-...-012). Wheeler edited variant. *(reconciled 2026-05-20 WI-59 — copy locked by `BurnTimeCalculatorTests.approvedMainScreenSafetyCopyIsCaptured` (`skinTypePickerPrompt` exact-match), `skinTypePickerHeaderIsBehaviorFirstPerWheelerSpec`, and `skinTypePickerSubtextCapturesBehaviorCuesAndRangeOfTones`; earlier spec quote "Pick the row that matches what your skin does, not its color" was a paraphrase of the picker header + subtext combined and never matched a shipped string.)* |
...
7. **L1 disclaimer link** — inline bottom-of-content link: `Informational only. Not medical advice.` (rendered as a SwiftUI `Label` with `info.circle` SF Symbol inside a `NavigationLink` that opens `AboutView(highlightEstimateApplicability: true)` — `ProductCopy.disclaimerLinkLabel` at `AppViews.swift:1757`; the design arrow `→` was a spec-time link indicator, not a literal rendered character, following the same convention as the LANE 2 #4 hero-card caveat arrow noted above). *(reconciled 2026-05-20 WI-60 — copy locked by `BurnTimeCalculatorTests.approvedMainScreenSafetyCopyIsCaptured` (`disclaimerLinkLabel` exact-match).)*
...
*Last reconciled with code: `9fd32fb` — 2026-05-20 (WI-59: §LANE 1 row 3 picker prompt + subtext now quote the shipped `ProductCopy.skinTypePickerPrompt` / `skinTypePickerSubtext` strings verbatim; WI-60: §LANE 2 #7 footer disclaimer link drops the spec-time `→` arrow to match `ProductCopy.disclaimerLinkLabel` and the rendered `Label(systemImage: "info.circle")` at `AppViews.swift:1757`)*
