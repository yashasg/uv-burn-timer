...
### 2026-05-22T23:55:00Z — WI-L31-DEBT-04 shipped (PR #126, squash-merge): ADR-0001 line-citation AST auto-refresh

**Outcome:** PR #126 merged on `main`. ADR-0001 citations now resolve via a symbolic anchor manifest instead of literal line numbers — `test_S5_adr0001CitationsResolveToAstAnchors` reads the manifest, opens each cited file, and asserts the anchor still resolves. Legacy guard renamed to `test_S5_legacy_adr0001CitationsMatchLiveSourceLineNumbers` and marked `@available(*, deprecated)` for one-iteration soak.

**Approach:** (A) — structured citation manifest added as a markdown table in ADR-0001's References § (16 rows, ~42 lines). Rejected (B) (prose-parser + nearest-enclosing-decl) because the prose-parser itself becomes a maintenance hazard. Chose substring-resolved-against-source over true SwiftSyntax-decl-walk because SwiftSyntax lives in `tools/swiftlint-rules/` (ADR-0003) and is not linked into the iOS test target — adding it to `app/Package.swift` + `app.xcodeproj` is more invasive than the bug warrants. The substring anchors (e.g. `struct HeroTimerCard: View`, `.sheet(isPresented: $showSettings)`) are decl-equivalent at this granularity: each one is a syntactic landmark a SwiftSyntax decl walk would also pin to.

**Learnings:**

- **Citation manifests beat citation prose.** Putting the load-bearing test contract into a structured markdown table (with strict pipe-cell parsing in the test) lets future authors safely edit the surrounding ADR prose without breaking CI. The historical refresh-narrative addenda ("bumped from 2150 → 2171") can stay as fossilised records because the test no longer parses them.
