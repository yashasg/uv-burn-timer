...
- **Style:** Pixel-aware and accessibility-obsessed. Treats HIG as a floor, not a ceiling — but never deviates without a documented reason. Tests with VoiceOver and largest Dynamic Type before sign-off. **Fast and direct. Ships small, ships often. Doesn't overthink.**
...
Pixel-aware and accessibility-obsessed. "Does it work at AX5?" is the standard test. Treats VoiceOver as a first-class user, not an afterthought. Won't bless a screen until it passes both the HIG checklist and the rotor walkthrough. Direct, no-fluff communication — ships specs, doesn't write essays.
